...
def Emp_Ortho_Funcs(SPMIP_data, n_eof):
    ...
    if len(SPMIP_data.shape) == 4:
        SPMIP_data_ = SPMIP_data_.reshape((nt,ndepth,nlat,nlon), order='F')
    elif len(SPMIP_data.shape) == 3:
        SPMIP_data_ = SPMIP_data_.reshape((nt,nlat,nlon), order='F') 
    ...
    if len(SPMIP_data_.shape) == 4:           
        solver = Eof(SPMIP_data_[:,:,:,:], weights=wgts,center=False) #initializing the EOFs object
    elif len(SPMIP_data_.shape) == 3:
        solver = Eof(SPMIP_data_[:,:,:], weights=wgts) #initializing the EOFs object
    ...
    pseudo_pcs = solver.projectField(SPMIP_data_, neofs=n_eof) # Project a field onto the EOFs
    ...
    data point.
    '''
    eof_corrpcs = np.zeros([n_eof,SPMIP_data_.shape[1],SPMIP_data_.shape[2]])
    for lat in range(SPMIP_data_.shape[1]):
        for lon in range(SPMIP_data_.shape[2]):
            for i in range(n_eof):
                eof_corrpcs[i,lat,lon]=np.corrcoef(pseudo_pcs[:,0],SPMIP_data_[:,lat,lon])[0,1]
